  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: GetBuilder<ProfileController>(builder: (profileController) {
        return BodyWidget(
          appBar: AppBarWidget(title: 'profile'.tr,showBackButton: true),
          body: SingleChildScrollView(
            ...
            child: Column(children:  [
                      ...
                      child: Row( children: [
                        ...
                        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          Row(children: [
                            ...
                            SizedBox(width: 15, height : 15, child: ImageWidget(image: "${AppConstants.baseUrl}/storage/app/public/customer/level/${profileController.profileModel!.data!.level?.image}"))
                          ]),
              ...
              ProfileMenuItem(title: 'notification', icon: Images.notificationActive,
              ...
              ProfileMenuItem(title: 'privacy_policy', icon: Images.privacyPolicyIcon,
                  onTap: () => Get.to(() =>  PolicyScreen(isPolicy: true, image: Get.find<ConfigController>().config?.privacyPolicy?.image??'',))),

              ProfileMenuItem(title: 'terms_and_condition', icon: Images.termsAndCondition,
                  onTap: () => Get.to(() =>  PolicyScreen(image: Get.find<ConfigController>().config?.termsAndConditions?.image??''))),

              ProfileMenuItem(title: 'legal', icon: Images.privacyPolicy,
                  onTap: () => Get.to(() =>  PolicyScreen(isLegal: true, image: Get.find<ConfigController>().config?.legal?.image??''))),
              ...
              ProfileMenuItem(title: 'permanently_delete_account'.tr, icon: Images.deleteAccountIcon, divider: false,
                onTap: () {showDialog(context: context, builder: (_) {
                  return GetBuilder<AuthController>(builder: (authController){
                    return ConfirmationDialogWidget(
                      icon: Images.profileLogout,
