...
class _ConversationBubbleState extends State<ConversationBubble> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: isMe ? CrossAxisAlignment.end : CrossAxisAlignment.start, children: [
      Padding(padding: isMe ? const EdgeInsets.fromLTRB(20, 5,10, 4) : const EdgeInsets.fromLTRB(10, 5, 20, 4),
        child: Column(crossAxisAlignment: isMe ? CrossAxisAlignment.start : CrossAxisAlignment.end,
          children: [
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.max,
              ...
              children: [
                isMe ? const SizedBox() : ClipRRect(borderRadius: BorderRadius.circular(50),
                ...
                Flexible(child: Column(crossAxisAlignment: isMe ? CrossAxisAlignment.end:CrossAxisAlignment.start,
                  mainAxisSize: MainAxisSize.min, children: [
                    if(widget.message.message != null)
                      Flexible(child: Padding(padding: isMe ? EdgeInsets.only(left:Get.width *0.15 ) :  EdgeInsets.only(right:Get.width *0.1),
                        child: Container(decoration: BoxDecoration(
                            color: isMe ? Theme.of(context).primaryColor.withOpacity(0.10) : Theme.of(context).hintColor.withOpacity(0.10),
                    ...
                    SizedBox(width: widget.message.conversationFiles!.length < 4 ? context.width : context.width *0.6,
                      child: Directionality(textDirection:Get.find<LocalizationController>().isLtr ? isMe ?
                       TextDirection.rtl : TextDirection.ltr : isMe ? TextDirection.ltr : TextDirection.rtl,
                        child: GridView.builder(gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(childAspectRatio: 1,
                          ...
                          itemBuilder: (BuildContext context, index) {
                            ...
                            return isImage ? Padding( padding: const EdgeInsets.only(right: 5,top: 0,bottom: 5),
      ...
      Padding(padding: isMe ? const EdgeInsets.fromLTRB(5, 0, 10, 5) : const EdgeInsets.fromLTRB(50, 0, 5, 5),
        child: Text(DateConverter.isoDateTimeStringToDifferentWithCurrentTime(widget.message.createdAt??DateTime.now().toString()),
