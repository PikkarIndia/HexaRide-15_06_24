...
class MapController extends GetxController implements GetxService {
  ...
  void setFromToMarker(LatLng from, LatLng to,{bool isBound = true}) async{
    markers = HashSet();
  ...
  void setMarkersInitialPosition(){
    if(Get.find<RideController>().encodedPolyLine.isNotEmpty) {
      ...
      setFromToMarker(
          ...
          LatLng(markers[markers.length-1].latitude, markers[markers.length-1].longitude), isBound: false
      );
    }
  }
